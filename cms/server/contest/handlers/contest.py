    ...
    def render_params(self):
        ...
        if self.current_user is not None:
            ...
            for t_iter in self.contest.tasks:
                score_list[t_iter.name] = task_score(self.current_user, self.get_task(t_iter.name), public=True, rounded=True)[0]
                total_score = total_score + score_list[t_iter.name]
                total_score_max = total_score_max + t_iter.active_dataset.score_type_object.max_public_score
            ret["score_list"] = score_list
            ret["total_score"] = total_score
            ret["total_score_max"] = total_score_max
